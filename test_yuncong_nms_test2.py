...
parser.add_argument('--resume', default='./weights/Res50_pyramid.pth')
parser.add_argument('--our', default='./weights/best_our_Res50_pyramid_aug.pth')
parser.add_argument('--general', default='./weights/best_mod_Res50_pyramid.pth')
# parser.add_argument('--partab_mall', default='./weights/partab_mall.pth')
...
if __name__ == '__main__':
    ...
    writepath = "best_mod_our"+datetime.datetime.now().strftime('%Y%m%d_%H%M%S') + ".txt"
    ...
    with open(readpath,'r') as fr:
        with open(writepath,'w') as fw:
            for line in fr.readlines():
                ...
                print(test_path)
                # if ('our' in test_path or 'UCSD' in test_path) :
                ...
                net.eval()
                print('Finished loading model!')
                # elif ('Part_B' in test_path or 'Part_A' in test_path or 'Mall' in test_path):
                #     print('Loading model: partab_mall ..')
                #     ssd_net = build_sfd('test', 640, 2)
                #     net = ssd_net
                #     net.load_state_dict(torch.load(args.partab_mall))
                #     net.cuda()
                #     net.eval()
                #     print('Finished loading model!')
#                else:
