...
def train():
    ...
    for iteration in range(args.start_iter, max_iter):
        ...
        if (loss.data[0] < min_loss):
            ...
            torch.save(ssd_net.state_dict(), args.save_folder + 'best_our_Res50_pyramid_aug' + '.pth')
        loss.backward()
        optimizer.step()
        t2 = time.time()
        ...
        if iteration % 500 == 0 or iteration in stepvalues:
            ...
            torch.save(ssd_net.state_dict(), args.save_folder + 'our_Res50_pyramid_aug_' +
                       repr(iteration) + '.pth')
    torch.save(ssd_net.state_dict(), args.save_folder + 'our_Res50_pyramid_aug' + '.pth')
